...
export const ja = defineLocale({
  ...
  boot: {
    ready: 'Mousai Workspace の準備ができました',
    ...
    steps: {
      ...
      startingHermesDesktop: 'Mousai Workspace を起動中…'
    },
    errors: {
    ...
    failure: {
      title: 'Mousai Workspace を起動できませんでした',
      description:
  ...
  notifications: {
    ...
    errors: {
      ...
      methodNotAllowed:
        'デスクトップバックエンドがそのリクエストを拒否しました (405 Method Not Allowed)。Mousai Workspace を再起動してください。',
  ...
  settings: {
    ...
    gateway: {
      ...
      intro:
        'Mousai Workspace はデフォルトで独自のローカルゲートウェイを起動します。別のマシンや信頼できるプロキシの背後で既に動作している Hermes バックエンドをこのアプリで制御する場合は、リモートゲートウェイを使用してください。ゲートウェイ接続はマシン単位の設定で、プロファイルは接続したゲートウェイから検出されます。',
      ...
      restartingMessage: 'Mousai Workspace は保存された設定を使用して再接続します。',
      ...
      sshErrPlatform:
        'サポートされていないリモートプラットフォームです。Mousai Workspace の SSH モードは Linux、macOS、Windows のリモートホストに対応しています。',
  ...
  install: {
    ...
    setupChoiceTitle: 'Mousai Workspace をセットアップ',
    setupChoiceDesc:
    ...
    localStartUnavailable:
      'ローカルインストールを開始できません。Mousai Workspace を再起動して、もう一度お試しください。',
    remoteSetupTitle: '既存の Hermes に接続',
    remoteSetupDesc:
      'ゲートウェイ URL を入力してください。Mousai Workspace がトークンとブラウザーサインインのどちらが必要かを検出します。',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '画像を保存するには Mousai Workspace を再起動してください。',
    restartToSaveImages: '画像を保存するには Mousai Workspace を再起動してください',
