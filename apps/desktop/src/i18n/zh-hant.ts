...
export const zhHant = defineLocale({
  ...
  boot: {
    ...
    steps: {
      ...
      startingHermesDesktop: '正在啟動 Mousai Workspace…'
    },
    errors: {
  ...
  notifications: {
    ...
    errors: {
      ...
      methodNotAllowed: '桌面後端拒絕了該請求 (405 Method Not Allowed)。請嘗試重新啟動 Mousai Workspace。',
  ...
  settings: {
    ...
    gateway: {
      ...
      envOverride: '環境變數覆寫',
      intro:
        'Mousai Workspace 預設會啟動自己的本機閘道。如果您希望此應用程式控制另一台機器或可信代理後面已執行的 Hermes 後端，請使用遠端閘道。閘道連線屬於本機層級設定；設定檔是從已連線的閘道中探索出來的。',
      ...
      savedTitle: '閘道設定已儲存',
      restartingMessage: 'Mousai Workspace 將使用已儲存的設定重新連線。',
      ...
      sshErrPlatform: '不支援的遠端平台。Mousai Workspace 的 SSH 模式支援 Linux、macOS 和 Windows 遠端主機。',
  ...
  install: {
    ...
    setupChoiceTitle: '設定 Mousai Workspace',
    ...
    localStartUnavailable: '無法啟動本機安裝。請重新啟動 Mousai Workspace 後再試一次。',
    remoteSetupTitle: '連線到現有 Hermes',
    remoteSetupDesc: '輸入閘道 URL。Mousai Workspace 會偵測需要權杖還是瀏覽器登入。',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '重新啟動 Mousai Workspace 後可使用儲存圖片。',
    restartToSaveImages: '重新啟動 Mousai Workspace 以儲存圖片',
